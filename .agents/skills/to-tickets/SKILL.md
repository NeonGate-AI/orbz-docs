---
name: to-tickets
description: Decompose one ready Orbz Docs spec into vertical GitHub issues with explicit blocking edges.
...
6. Present the proposed ticket graph before publication. Confirm granularity, blocking edges and any merge/split decisions with the owner unless the current request already contains explicit approval to publish that exact breakdown.
7. Create GitHub issues in dependency order. Each issue links the canonical `.spec.md` path, names the public seam, contains checkable criteria and references its blockers.
8. Do not copy the entire spec into every issue, create speculative abstractions, or alter the parent spec while publishing tickets.
