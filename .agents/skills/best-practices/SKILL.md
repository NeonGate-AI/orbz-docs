...
Apply these practices through the existing Next.js 16 + Nextra 4 stack. Generic examples for Vite, Babel, alternate routers or client-state libraries are illustrative only and must not be introduced unless an approved spec requires them. Start with `pnpm harness:check`, `pnpm check:static` and the production build; inspect headers/deployment behavior separately when the finding depends on runtime infrastructure.
