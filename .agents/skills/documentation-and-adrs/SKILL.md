...
Durable engineering context lives under `.agents/`: rules define normative constraints, ADRs record architectural decisions, numbered specs define behavioral/change contracts and `.audits/` provides mechanical evidence. Public product documentation lives under `content/` and must describe the published `@neongate-ai/orbz` contract. Keep internal harness prose out of public Nextra pages unless it is genuinely part of the user-facing documentation.
