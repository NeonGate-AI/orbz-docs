...
This is a Next.js 16 + Nextra 4 documentation application, not a generic SPA. Prefer Server Components and MDX for content; add Client Components only for interaction that cannot be expressed server-side. Preserve Nextra's semantic/navigation behavior before replacing theme UI. WCAG 2.2 AA is the accessibility target, and performance work follows the layered evidence model in `.agents/context/web-quality.md`.

Examples in this skill are design patterns, not permission to introduce Tailwind, a state library, Storybook or another UI framework. Reuse the dependencies and CSS conventions already present in the repository unless an approved spec explicitly changes them.
...
Use 24 by 24 CSS pixels as the WCAG 2.5.8 minimum where its exceptions do not apply, and 44 by 44 CSS pixels as a larger practical target for touch controls where appropriate.
