...
```html
<orb-z state="idle" preset="gojhonny" size="300px"></orb-z>
```
...
```vue
<orb-z :state="state" :size="`${size}px`" preset="gojhonny" />
```
...
```svelte
<orb-z {state} size={`${size}px`} preset="gojhonny"></orb-z>
```
...
@Component({
  ...
  selector: "app-root",
  template: `<orb-z [attr.state]="state" preset="gojhonny"></orb-z>`
})
