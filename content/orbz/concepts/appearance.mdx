...
`gojhonny` is the default appearance. This remains true when the `preset`
attribute is absent, which lets custom mode inherit `gojhonny` colors for any
...
Custom colors are partial. Missing values inherit the default `gojhonny` color,
