...
| `preset` | `preset` | `gojhonny`, `periwinkle`, `magenta`, `peach`, `mocha`, `ivory` | `gojhonny` colors |
| `color-primary` | — | CSS color | `gojhonny` primary |
| `color-secondary` | — | CSS color | `gojhonny` secondary |
| `color-accent` | — | CSS color | `gojhonny` accent |
| `color-highlight` | — | CSS color | `gojhonny` highlight |
| `color-background` | — | CSS color | `gojhonny` core color |
...
An invalid preset normalizes to `gojhonny`. The public API is named `preset`;
...
Empty values are removed and missing colors use the `gojhonny` defaults. An
...
`gojhonny` when the attribute is absent. Use `hasAttribute("preset")` when you
